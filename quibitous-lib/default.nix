...
let
  ...
  mkConfig = environment: let
    envVersion = environment.packages.quibitous.version;
    defaultLogConfig = {
      level = "info";
      format = "plain";
      output = "stderr";
  ...
  mkConfigHydra = environment: runCommand "quibitous-config" {
      buildInputs = [ environment.packages.quibitous ];
    } ''
    mkdir -p $out/nix-support
    ${jq}/bin/jq . < ${__toFile "quibitous-config.yaml" (__toJSON (mkConfig environment))} > $out/config.yaml
    ...
    echo "file binary-dist $out/quibitous-version.txt" >> $out/nix-support/hydra-build-products
    quibitous --full-version > $out/quibitous-version.txt
  '';

  linkTo = rev:
    if __stringLength rev == 40 then
      "https://github.com/The-Blockchain-Company/quibitous/commits/${rev}"
    else
      "https://github.com/The-Blockchain-Company/quibitous/releases/tag/${rev}";
  ...
  mkConfigHtml = environments: runCommand "quibitous-html" { buildInputs = [ jq ]; } ''
    mkdir -p $out/nix-support
    ...
    echo "report quibitous $out index.html" > $out/nix-support/hydra-build-products
  '';
  ...
  environments = {
    itn_rewards_v1 = {
      ...
      block0bin = fetchurl {
        url = "https://update-quibitous-incentivized.tbco.io/block-0.bin";
    ...
    legacy = {
      ...
      registryUrl = "https://explorer.legacy.quibitous-testnet.tbcodev.io/stakepool-registry/registry.zip";
      syncTolerance = "600s";
      trustedPeers = [
        {
    ...
    beta = {
      ...
      registryUrl = "https://explorer.beta.quibitous-testnet.tbcodev.io/stakepool-registry/registry.zip";
      syncTolerance = "300s";
      trustedPeers = [
        {
    ...
    nightly = {
      ...
      registryUrl = "https://github.com/The-Blockchain-Company/testnet-stakepool-registry/archive/master.zip";
      syncTolerance = "300s";
      trustedPeers = [
        {
    ...
    qa = {
      ...
      registryUrl = "https://explorer.qa.quibitous-testnet.tbcodev.io/stakepool-registry/registry.zip";
      syncTolerance = "300s";
      trustedPeers = [
        {
