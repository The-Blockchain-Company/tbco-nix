...
let
  ...
  deprecationWarning = parameter: builtins.trace ''
    WARNING: tbco-nix \"${parameter}\" parameter is deprecated.
  ...
  commonLib = rec {
    ...
    inherit (import sources.bcc-repo-tool {inherit system;}) bcc-repo-tool;
    stack-cabal-sync-shell = pkgsDefault.callPackage ./pkgs/stack-cabal-sync-shell.nix { inherit bcc-repo-tool; };
  ...
  bccLib = commonLib.pkgsDefault.callPackage ./bcc-lib {};
  quibitousLib = commonLib.pkgsDefault.callPackage ./quibitous-lib { inherit rust-packages; };
  ...
  overlays = {
    ...
    tbcoNix = [(pkgs: super: rec {
      tbcoNix = import ./. {
        inherit (pkgs) config system;
        pkgsDefault = pkgs;
      };
      tbco-nix.lib = import ./lib pkgs.lib;
    })];
  ...
  # This attribute is here for tbco-nix/release.nix Hydra builds.
  ...
  in rec {
    ...
    (pkg: throw "ERROR: tbco-nix `haskell-nix-extra-packages.${pkg}` has been removed.");
